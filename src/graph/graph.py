...
Please provide the relationship you can determine from the image.
    ...
    def set_image_goal(self, image):
        print("--- set_image_goal: 开始设置图像目标 ---")
        if isinstance(image, np.ndarray):
            print("set_image_goal: 检测到 numpy 数组，正在转换为 PIL Image。")
            image = Image.fromarray(image)
            print("set_image_goal: 转换为 PIL Image 完成。")
        self.instance_imagegoal = image
        
        print("set_image_goal: 正在调用 VLM 进行图像到文本的转换...")
        text_goal = self.vlm(self.prompt_image2text, self.instance_imagegoal)
        print(f"set_image_goal: 图像到文本转换完成。提取的文本目标: '{text_goal}'")
        
        print("set_image_goal: 正在调用 set_text_goal 设置文本目标...")
        self.set_text_goal(text_goal)
        print("set_image_goal: set_text_goal 调用完成。")
        print("--- set_image_goal: 图像目标设置完成 ---")

    def set_text_goal(self, text_goal):
        print("--- set_text_goal: 开始设置文本目标 ---")
        if isinstance(text_goal, dict):
            print("set_text_goal: 检测到字典格式的文本目标，正在组合属性。")
            text_goal_original = text_goal # Store original for logging
            text_goal = text_goal['intrinsic_attributes'] + ' ' + text_goal['extrinsic_attributes']
            print(f"set_text_goal: 组合前字典: {text_goal_original}，组合后文本目标: '{text_goal}'")
        self.text_goal = text_goal
        
        print(f"set_text_goal: 正在使用 '{self.text_goal}' 构建目标图 (goalgraph)...")
        self.goalgraph = self.graphbuilder.build_graph_from_text(text_goal)
        print("set_text_goal: 目标图 (goalgraph) 构建完成。")
        
        print("set_text_goal: 正在对目标图进行分解 (goal_decomposition)...")
        self.goalgraph_decomposed = self.goalgraphdecomposer.goal_decomposition(self.goalgraph)
        print("set_text_goal: 目标图分解完成。")
        print("--- set_text_goal: 文本目标设置完成 ---")
    ...
    def get_grounded_sam(self, device):
        ...
        model = load_model(self.groundingdino_config_file, self.groundingdino_checkpoint, local_bert_path, device)
